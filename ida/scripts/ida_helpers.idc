...
{
	if (name == "CheckObjectVisibleWide") {
...
static DeleteSegment(start, end)
{
	auto addr;
	for (addr = start; addr < end; addr = addr + 1) {
		MakeUnkn(addr, 0);
		MakeName(addr, "");
		SetManualInsn(addr, "");
	}
...
static CreateSegment(start, end, name)
{
	SegCreate(start, end, 0, 1, 1, 0);
	SegRename(start, name);
}